...
lazy_static::lazy_static! {
    static ref STORE_COMMON: Arc<RwLock<Store>> = {
        Arc::new(RwLock::new(Store::new(&paths().data_path().join("common.json"), true)))
    };
    static ref STORE_SETTINGS: Arc<RwLock<Store>> = {
        Arc::new(RwLock::new(Store::new(&paths().data_path().join("settings.json"), true)))
    };
    static ref STORE_LOCAL_COMMON: Arc<RwLock<Store>> = {
        Arc::new(RwLock::new(Store::new(&paths().local_data_path().join("common.json"), true)))
    };
    static ref STORE_LOCAL_SETTINGS: Arc<RwLock<Store>> = {
        Arc::new(RwLock::new(Store::new(&paths().local_data_path().join("settings.json"), true)))
    };
}
