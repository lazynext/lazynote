...
const handleIcon = async (iconPath, outputPath, rounded, size, blankSpace) => {
...
const resizeIcon = async (inputPath, outputPath, size) => {
  return handleIcon(inputPath, outputPath, false, size);
};
...
const generateAndroidIcon = async (inputPath, outputPath) => {
  const targets = [
    {
      name: 'hdpi',
      size: 49,
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    await resizeIcon(inputPath, path.join(outFolder, fileName), {
...
const generateIOSIcon = async (inputPath, outputPath) => {
  const targets = [
    {
      size: 20,
      multipliers: [1, 2, 3],
...
const createIcons = async () => {
  ...
  await generateIOSIcon(iconPath, iosPath);
};
